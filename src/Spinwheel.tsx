...
type Discount = { value: number | 'GOLD'; text: string };
...
const Wheel = styled('div')<{ spinning: boolean; rotation: number }>(({ spinning, rotation }) => ({
  position: 'relative',
  width: '320px',
  height: '320px',
  borderRadius: '50%',
...
const Segment = styled('div')<{ rotate: number; color: string }>(({ rotate, color }) => ({
  position: 'absolute',
  width: '50%',
  height: '50%',
  right: '0',
...
const SpinWheel: React.FC = () => {
  ...
  const spinAudioRef = useRef<HTMLAudioElement>(new Audio(spinSound));
